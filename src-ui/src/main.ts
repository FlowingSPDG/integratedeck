...
async function onSlotClick(page: Page, row: number, col: number, existing?: Slot) {
  ...
  if (!slot) {
    ...
    await refresh()
    slot = Object.values(page.slots).find(
      (s) => s.locator.row === row && s.locator.column === col,
    ) ?? slot
  }
...
async function loadPlugins() {
  const scan = await invoke<{ streamdeck: string[]; companion: string[] }>('scan_plugins')
  ...
  sdList.querySelectorAll('li[data-path]').forEach((li) => {
    li.addEventListener('click', async () => {
      const path = decodeURIComponent((li as HTMLElement).dataset.path!)
      const loaded = await invoke<{ plugin_uuid: string; port: number; name: string }>(
        'load_sd_plugin',
        { path },
      )
      statusEl.textContent = `Loaded ${loaded.name} on port ${loaded.port}`
      ;(document.getElementById('plugin-uuid') as HTMLInputElement).value = loaded.plugin_uuid
    })
  })
...
async function loadConnections() {
  const list = await invoke<{ id: string; label: string; module_id: string }[]>('list_connections')
...
document.getElementById('btn-add-connection')!.addEventListener('click', async () => {
  ...
  if (ping) statusEl.textContent = `Sidecar ping sent: ${ping}`
})
